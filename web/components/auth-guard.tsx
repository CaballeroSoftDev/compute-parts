    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          <div className="mx-auto h-8 w-8 animate-spin rounded-full border-b-2 border-[#007BFF]"></div>
...
export function AdminGuard({ children }: { children: React.ReactNode }) {
  return <AuthGuard allowedRoles={['admin', 'superadmin']}>{children}</AuthGuard>;
}
...
export function ClientGuard({ children }: { children: React.ReactNode }) {
  return <AuthGuard allowedRoles={['cliente', 'admin', 'superadmin']}>{children}</AuthGuard>;
}
