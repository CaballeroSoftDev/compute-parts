...
export class UploadService {
  ...
  static async deleteImage(filePath: string, bucket?: string): Promise<{ success: boolean; error?: string }> {
    try {
      const { error } = await supabase.storage.from(bucket || this.BUCKETS.PRODUCT_IMAGES).remove([filePath]);
  ...
  static async deleteUserAvatar(userId: string, avatarUrl?: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (!filePath) {
        ...
        const { data: files, error: listError } = await supabase.storage.from(this.BUCKETS.USER_AVATARS).list(userId);
        ...
        if (files && files.length > 0) {
          ...
          const { error: deleteError } = await supabase.storage.from(this.BUCKETS.USER_AVATARS).remove(filePaths);
        ...
        const { error: deleteError } = await supabase.storage.from(this.BUCKETS.USER_AVATARS).remove([filePath]);
  ...
  static getPublicUrl(filePath: string, bucket?: string): string {
    const { data } = supabase.storage.from(bucket || this.BUCKETS.PRODUCT_IMAGES).getPublicUrl(filePath);
  ...
  static async listFiles(folder?: string, bucket?: string): Promise<{ files: string[]; error?: string }> {
    try {
      const { data, error } = await supabase.storage.from(bucket || this.BUCKETS.PRODUCT_IMAGES).list(folder || '');
  ...
  static validateImageFile(
    file: File,
    type: 'category' | 'brand' | 'product' | 'avatar' | 'general' = 'general'
  ...
  static async uploadProductImages(files: File[], productId: string, options?: UploadOptions): Promise<UploadResult[]> {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const fileExt = file.name.split('.').pop()?.toLowerCase();
        const fileName =
          options?.fileName || `${Date.now()}-${i}-${Math.random().toString(36).substring(2)}.${fileExt}`;
        const folder = options?.folder || `products/${productId}`;
        const filePath = `${folder}/${fileName}`;
